...
better-auth 部分的表，是 [better-auth](https://www.better-auth.com/) 库所需要依赖的表。

其中包含了：`user`、`account`、`session`、`verification` 、`two_factor` 五张表。

## prisma

`_prisma_migrations` 表是 prisma 进行 migrate 需要的表，请不要擅自修改！
